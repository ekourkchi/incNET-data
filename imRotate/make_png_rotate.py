...
def make_small_PS(db_dir, pgc, ra, dec, d25, angle):
    ...
    for band in ['g', 'r', 'i']:
        ...
        cmd = 'cp '+pgc_band_root+'/pgc' + \
            str(pgc)+'_'+band+'.fits '+pgc_band_root+'/tmp/.'
        xcmd(cmd, True)
      ...
      '''
        ...
        # size = 0.1  # degree
            ...
            if size > 3:
                size = 3.
            file = 'tmp.fits'
            cmd = 'mRotate -r '+str(angle)+' '+file+' ' + \
                file+' '+str(ra)+' '+str(dec)+' '+str(size)
            xcmd(cmd, True)

            xcmd('mv tmp.fits '+'pgc'+str(pgc) +
                 '_d25x2_rot_'+band+'.fits', True)
...
table = np.genfromtxt(inFile, delimiter='|',
...
for i in range(0, len(pgc)):
    ...
    if pgc[i] in id_lexi and sdss[i] == 1:
        ...
        ellipsefile = db_dir+'/data/' + \
            ra_db(ra[i])+'/photometry/'+'/pgc'+str(pgc[i])+'_i_ellipsepar.dat'

        if sdss[i] == 0:  # or not os.path.isfile(ellipsefile):
            continue

        if os.path.isfile(ellipsefile):
            ra_cen, dec_cen, semimajor, semiminor, PA_gal = get_ellipse(
                ellipsefile)
        ...
        if True:
            xcmd('cp -rf '+dataDir+'/pgc' +
                 str(pgc[i])+'_*.fits '+db_dir+'/.', True)
            xcmd('cp -rf '+dataDir+'/pgc' +
                 str(pgc[i])+'_*.fits.gz '+db_dir+'/.', True)
            ...
            cmd = 'sh ds9_fits2jpeg_rotate.sh '+db_dir + ' pgc' + \
                str(pgc[i]) + ' ' + str(d25[i]) + ' ' + str(90.-PA_gal)+' &'
...
# BEGIN Get Colorful images directly from SDSS
            ...
            cmd = 'bash cutoutFromSDSS.bash ' + \
                str(ra_cen)+' '+str(dec_cen)+' '+str(npix) + \
                ' '+str(90.-PA_gal)+' '+db_dir+' &'

            print cmd

            xcmd(cmd, True)
            xcmd('mv '+db_dir+'/test.jpg '+db_dir+'/SDSS_PNG_rotate_finale/' +
                 'pgc'+str(pgc[i])+'_d25x2_rot_gri.jpg', True)
